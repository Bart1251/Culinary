...
export const EditRecipe = () => {
    ...
    const [recipe, setRecipe] = useState<RecipeEditData>({
        id: -1,
        name: "",
        description: "",
        categoryId: -1,
    ...
    function addIngredient() {
        if (recipe.ingredients.filter(i => i.name == ingredient.name).length == 0 &&
            recipe.newIngredients.filter(i => i.name == ingredient.name).length == 0 &&
            ingredient.name.length > 0 &&
            ingredient.amount > 0 &&
            ingredient.unitId != -1) {
            setRecipe({ ...recipe, newIngredients: [...recipe.newIngredients, ingredient] });
            setIngredient({
                name: "",
                amount: 0,
                unitId: -1
    ...
    function addStep() {
        if (step.length > 0 &&
            recipe.steps.filter(s => s.content == step).length == 0 &&
            recipe.newSteps.filter(s => s == step).length == 0) {
            setRecipe({ ...recipe, newSteps: [...recipe.newSteps, step] });
            setStep("");
        }
    }
    ...
    function removeIngredient(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        setIngredientsToDelete([
            ...
            recipe.ingredients.filter(
                i => i.name == Array.from((e.target as HTMLButtonElement).parentElement!.children).find(e => e.classList.contains("ingredient"))?.textContent)[0].id
        ]);
        setRecipe({
    ...
    function removeStep(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        setStepsToDelete([
            ...stepsToDelete,
            recipe.steps.filter(
                i => i.content == Array.from((e.target as HTMLButtonElement).parentElement!.children).find(e => e.classList.contains("step"))?.textContent)[0].id
        ]);
        setRecipe({
            ...recipe,
            steps: recipe.steps.filter(
                i => i.content != Array.from((e.target as HTMLButtonElement).parentElement!.children).find(e => e.classList.contains("step"))?.textContent
            )
        });
    }

    function removeNewIngredient(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        setRecipe({
            ...recipe, newIngredients: recipe.newIngredients.filter(
                i => i.name != Array.from((e.target as HTMLButtonElement).parentElement!.children).find(e => e.classList.contains("ingredient"))?.textContent
            )
        });
    }

    function removeNewStep(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        setRecipe({
            ...recipe,
            newSteps: recipe.newSteps.filter(
    ...
    async function submit(e: React.FormEvent) {
        ...
        if (recipe.name.length > 0 &&
            ...
            recipe.categoryId != -1 &&
            recipe.difficulty != -1 &&
            (recipe.ingredients.length > 0 || recipe.newIngredients.length > 0) &&
            (recipe.steps.length > 0 || recipe.newSteps.length > 0)) 
        {
            await mutateAsync();
        }
    }
    ...
    return (
        <form onSubmit={submit} className="d-flex flex-column gap-3">
                                ...
                                {recipe.ingredients && recipe.ingredients.map((i, n) =>
                                    <li key={n}>
                                        <div className="d-flex flex-row align-items-center ingredient-list mb-2">
                                            ...
                                            <label>{i.amount + " " + units?.find(u => u.id == i.unit.id)?.name}</label>
                                            <button type="button" className="btn btn-outline-danger ms-3" onClick={(e) => removeIngredient(e)}>Usuń</button>
                                        </div>
                                    </li>
                                )}
                                {recipe.newIngredients && recipe.newIngredients.map((i, n) =>
                                    <li key={n + recipe.ingredients.length}>
                                        <div className="d-flex flex-row align-items-center ingredient-list mb-2">
                                            <label className="ingredient">{i.name}</label>
                                            <div className="dots flex-grow-1"></div>
                                            <label>{i.amount + " " + units?.find(u => u.id == i.unitId)?.name}</label>
                                            <button type="button" className="btn btn-outline-danger ms-3" onClick={(e) => removeNewIngredient(e)}>Usuń</button>
                                        </div>
                                    </li>
                                )}
                            </ul>
                                ...
                                {recipe.steps && recipe.steps.map((s, n) => 
                                    <li key={n}>
                                        <div className="d-flex flex-row align-items-center mb-2 ingredient-list">
                                            <label className="flex-grow-1 step">{s.content}</label>
                                            <button type="button" className="btn btn-outline-danger ms-3 justify-self-end" onClick={(e) => removeStep(e)}>Usuń</button>
                                        </div>
                                    </li>
                                )}
                                {recipe.newSteps && recipe.newSteps.map((s, n) =>
                                    <li key={n + recipe.steps.length}>
                                        <div className="d-flex flex-row align-items-center mb-2 ingredient-list">
                                            <label className="flex-grow-1 step">{s}</label>
                                            <button type="button" className="btn btn-outline-danger ms-3 justify-self-end" onClick={(e) => removeNewStep(e)}>Usuń</button>
                                        </div>
                                    </li>
                                )}
                            </ul>
            ...
            <button type="submit" className="btn btn-success fs-5">Edytuj przepis</button>
        </form>
    )
}
